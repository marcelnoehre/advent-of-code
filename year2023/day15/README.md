...
<p>The HASH algorithm is a way to turn any string of characters into a single <em>number</em> in the range 0 to 255. To run the HASH algorithm on a string, start with a <em>current value</em> of <code>0</code>. Then, for each character in the string starting from the beginning:</p>
<ul>
...
<li>Set the <em>current value</em> to the remainder of dividing itself by <code>256</code>.</li>
</ul>
...
<p>Along the wall running parallel to the boxes is a large library containing lenses organized by <em>focal length</em> ranging from <code>1</code> through <code>9</code>. The reindeer also brings you a small handheld label printer.</p>
