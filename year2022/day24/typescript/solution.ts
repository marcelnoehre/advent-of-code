...
const [start, end, maps] = [{ x: 1, y: 0, steps: 0 }, { x: arr[0].length - 2, y: arr.length - 1, steps: 0 }, []]; setup();
console.log(part_1());
console.log(part_2());
...
function setup(): void {
    ...
    arr.forEach((line, y) => {line.split('').map((cell, x) => {if (cell !== '.') map[`${x},${y}`] = (map[`${x},${y}`] || []).concat([cell])})});
    ...
    do { 
        ...
        Object.keys(map).forEach((key) => {
            ...
            map[key].forEach((dir) => {
                let pos = key;
                if (dir === '>') pos = `${x === arr[0].length - 2 ? 1 : x + 1},${y}`;
                if (dir === '<') pos = `${x === 1 ? arr[0].length - 2 : x - 1},${y}`;
                if (dir === 'v') pos = `${x},${y === arr.length - 2 ? 1 : y + 1}`;
                if (dir === '^') pos = `${x},${y === 1 ? arr.length - 2 : y - 1}`;
...
function neighbors(maps, { x, y, steps }) {
    return [{x: x - 1, y, steps: steps + 1},{x: x + 1, y, steps: steps + 1},{x, y: y - 1, steps: steps + 1},{x, y: y + 1, steps: steps + 1},{x, y, steps: steps + 1}
    ].filter(pos => !(maps[(steps + 1) % maps.length][`${pos.x},${pos.y}`] && maps[(steps + 1) % maps.length][`${pos.x},${pos.y}`].length));
}
