...
export function part_1(): number {
  return Object.values(filesystem).filter((size: number) => size <= 100000).reduce((acc: number, size: number) => acc + size, 0);
}

export function part_2(): number {
  return Math.min(...Object.values(filesystem).filter((size: number) => size >= (30000000 - (70000000 - filesystem['/']))));
}
