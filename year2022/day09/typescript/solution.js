...
function part_2(list) {
    var tailPositions = [[0, 0]];
    var head = [0, 0];
    var tail = Array.from({ length: 9 }, function () { return [0, 0]; });
    list.forEach(function (pair) {
        for (var i = 0; i < parseInt(pair[1]); i++) {
            head[0] += directions[pair[0]][0];
            head[1] += directions[pair[0]][1];
...
function checkTailPosition(tailPositions, start, end) {
    return !tailPositions.some(function (pos) { return pos[0] === start && pos[1] === end; });
}
