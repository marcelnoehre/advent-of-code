...
function simulate(iterations: number): number {
    ...
    for (let i = 0; i < iterations; ++i) {
        ...
        for (let j = 0; j < rocks[i % rocks.length].row.length; ++j) {
            for (let k = 0; k < rocks[i % rocks.length].row[j].length; ++k) {
                if (rocks[i % rocks.length].row[j][k] === '#') chamber[rowId - j][colId + k] = '#';
            }
        }
