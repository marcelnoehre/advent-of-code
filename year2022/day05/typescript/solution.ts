...
export function part_1(): string {
  ...
  instructions.forEach(move => stacks[move[5]-1].push(...stacks[move[3]-1].splice(stacks[move[3]-1].length - move[1], move[1]).reverse()));
  return stacks.map((_, i) => stacks[i].pop()).join('');
}

export function part_2(): string {
  let stacks: string[][] = initalStacks.map(stack => [...stack]);
  instructions.forEach(move => stacks[move[5] - 1].push(...stacks[move[3] - 1].splice(-move[1])));
  return stacks.map((_, i) => stacks[i].pop()).join('');
}

function setupStacks(arr: string[]) {
  let build: string[][] = [...Array(9)].map(() => []);
  for(let i = 1; i < arr[0].length; i += 4) arr.forEach(stack => { if(/[A-Z]/.test(stack.charAt(i))) build[Math.floor(i / 4)].push(stack.charAt(i))});
  return build;
}