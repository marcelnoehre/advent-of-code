...
export function part_1(): number {
  ...
  for (const row of input) if (!(row[0][0] !== row[1][0] && row[0][1] !== row[1][1])) [seen, duplicated] = checkRow(iterate(row), seen, duplicated);
  return duplicated.size;
}

export function part_2(): number {
  let [seen, duplicated] = [new Set<string>(), new Set<string>()];
  for (const row of input) [seen, duplicated] = checkRow(iterate(row), seen, duplicated);
  return duplicated.size;
}
...
function checkRow(iterator: Generator<[number, number]>, seen: Set<string>, duplicated: Set<string>): [Set<string>, Set<string>] {
  ...
  while(!iteration.done) {
    ...
    if (seen.has(coordinate)) duplicated.add(coordinate);
    seen.add(coordinate);
