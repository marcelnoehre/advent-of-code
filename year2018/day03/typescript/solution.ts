...
export function part_1(): number {
  ...
  input.forEach(([_, x, y, w, h]) => { for (let i = x; i < x + w; i++) for (let j = y; j < y + h; j++) if (fabric[i][j]++ === 1) overlaps++ });
  return overlaps;
}
...
export function part_2(): number {
  ...
  for (const [id, x, y, w, h] of input) {
    noOverlap.add(id);
    for (let i = x; i < x + w; i++) for (let j = y; j < y + h; j++) fabric[i][j] === 0 ? fabric[i][j] = id : (noOverlap.delete(id), noOverlap.delete(fabric[i][j]));
  }
